...
: "${FETE_DATA_ROOT:="${FETE_ROOTDIR}/data"}"
...
# Key for your IP address, obtained from https://fetedujour.fr/api
: "${FETE_KEY:=""}"
...
BEGIN:VEVENT
CLASS:PUBLIC
UID:$(date -u -d "$today" +'%Y%m%d')-${2}@fetedujour.fr
...
# page layout changes.
if [ -z "$FETE_KEY" ]; then
  [ -z "$FETE_IP" ] && FETE_IP=$(run_curl 'https://icanhazip.com' | tr -d '\n')
